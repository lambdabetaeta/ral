...
//!
//! Provides [`resolve_path`], a pure (no filesystem access) path
//! normaliser used throughout the evaluator to turn user-supplied
//! paths into absolute, `.`/`..`-free equivalents.

use std::path::{Component, Path, PathBuf};

/// Bidirectional firmlink/symlink pairs the kernel substitutes
/// transparently: a path under `a` and the same path under `b` denote
/// the same file.  macOS-only — empty elsewhere so [`path_aliases`] is
/// a no-op on Linux and Windows.
///
/// Used to keep the grant matcher correct when `canonicalize` is
/// unavailable — notably under Seatbelt, where `realpath(3)` can fail
/// on `/tmp` itself, causing comparisons to fall back to lexical form
/// on both sides.
#[cfg(target_os = "macos")]
...
/// Return `p` together with any alternate lexical forms that the host
/// filesystem treats as identical (e.g. `/tmp/foo` ↔ `/private/tmp/foo`
/// on macOS).  Pure: no filesystem access.
///
/// The matcher uses this so a grant authored as one form still covers
/// an access expressed as the other, even when `canonicalize` cannot
/// be relied on to bridge them.
...
/// Pure helper used by both the runtime grant matcher and the nested
/// grant intersector.
...
/// Like `str::strip_prefix`, but only matches when the prefix ends on a
/// path-component boundary — so `/tmp` does not pseudo-match `/tmpx`.
...
/// Resolve `path` against `cwd`, normalising `.` and `..` components.
///
/// If `path` is already absolute it is normalised in place.  Otherwise
/// it is joined to `cwd` (or to `std::env::current_dir` when `cwd` is
/// `None`).  The normalisation is purely lexical -- no symlink
/// resolution -- so the result may differ from `canonicalize`.
...
/// Walk `path` (a colon-separated `PATH` string) looking for an executable
/// file named `name`.  Returns the absolute path of the first hit, or
/// `None` if none of `path`'s directories contain an executable `name`.
///
/// Returns `None` immediately if `name` already contains a `/` — that
/// is treated as a path, not a bare command, and is not the business of
/// PATH lookup.
///
/// Both classify (`Shell::classify_command_head`) and check
/// (`evaluator::exec::exec_policy_names`) need the same resolution so a
/// path-keyed grant matches at both gates.
#[cfg(unix)]
...
#[cfg(not(unix))]
pub fn resolve_in_path(_name: &str, _path: &str) -> Option<String> {
    None
}
