...
///
/// Reads and writes consult the same deny set — there is one deny
/// region per layer, not two.  See SPEC §11.2.
///
...
pub(crate) fn check_fs_op(
    ...
    for fs in ctx.grants.fs() {
        ...
        let in_deny = fs
            .deny_paths
            .iter()
            .any(|d| crate::path::path_within(&resolved, &resolver.check(d.as_str())));
        if in_deny {
            denied = true;
            break;
        }
        let mut hit: Option<&str> = None;
        for prefix in op.prefixes(fs) {
            if crate::path::path_within(&resolved, &resolver.check(prefix.as_str())) {
    ...
    if has_fs_policy {
        emit_capability_audit(ctx, "fs", !denied, audit, site, |m| {
            ...
            if !denied && let Some(gp) = granted_prefix {
                m.insert("granted".into(), Value::String(gp));
            }
        });
    }
