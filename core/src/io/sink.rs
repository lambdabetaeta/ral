...
///
/// The REPL installs one of these at `shell.turn.io.stdout` so every write — from
...
/// `shell.turn.io.stdout`/`stderr` themselves.
...
/// to wire onto `shell.turn.io.{stdout,stderr}` *and* the buffer arc to drain
