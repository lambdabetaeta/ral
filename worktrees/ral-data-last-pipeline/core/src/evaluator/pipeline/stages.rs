...
/// re-deriving "byte vs value vs none" at every spawn site.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(super) enum Edge {
    /// Connects to a sibling stage via an OS byte pipe.
    Bytes,
    /// Connects to a sibling stage via an mpsc value channel.
    Value,
    /// No sibling on this side — first/last stage, or a value-only stage
    /// whose neighbour was eliminated by mode unification.
    None,
}
...
impl Edge {
    fn from_mode(mode: crate::ty::Mode) -> Self {
        match mode {
            ...
            // After `resolve_pipeline`'s default-resolve, anything that's
            // not Bytes is the value channel.
            _ => Self::Value,
        }
    }
}
...
pub(super) struct PipelinePlan {
    ...
    /// Every stage is Internal *and* every channel is value-mode — no
    /// bytes anywhere.  Such pipelines collapse to a sequential fold
    /// over `invoke::invoke`; no threads, no mpsc, no `PipelineGroup`.
    pub(super) pure_value_internal: bool,
}
...
pub(super) fn resolve_pipeline(
    ...
    // Pure-value-internal: no external stages, and every edge (including
    // the first stage's input and the last stage's output) is value-mode.
    // Such pipelines have no bytes to stream, no processes to manage,
    // and no concurrency to express — `run_value_only` evaluates them
    // as a left fold.
    let pure_value_internal = !specs.is_empty()
        && specs.iter().all(|s| {
            !s.dispatch.is_external()
                && s.comp_type.input != crate::ty::Mode::Bytes
                && s.comp_type.output != crate::ty::Mode::Bytes
        });
...
fn launch_internal_stage(
    ...
    // Apply the per-stage edge plan computed during `resolve_pipeline`:
    // the launcher does not re-derive byte/value routing.
    //
    // Each pipe end has exactly one owner from construction onward.  The
    // writer (and value-tx) are moved into the spawned thread; the reader
    // (and value-rx) become the outgoing channel.  No `try_clone`, no
    // `as_ref().transpose()` — ownership alone enforces "the parent holds
    // nothing extra."  Mismatched channels (predecessor produced bytes for
    // a value consumer or vice versa) are dropped silently, which unblocks
    // the sending side.
    let (incoming_stdin, incoming_value_rx) = match (incoming, spec.incoming) {
        (Channel::Bytes(r), Edge::Bytes) => (Some(r), None),
        (Channel::Value(rx), Edge::Value) => (None, Some(rx)),
        _ => (None, None),
    };
    ...
    let (val_tx, outgoing_value_rx) = match spec.outgoing {
        Edge::Value => {
    ...
    let handle = shell.spawn_thread(snap, move |child_env| {
        ...
        // Receive the upstream value (if this stage has a value edge in)
        // and hand it to `invoke` as the data-last argument.  Errors on
        // the value channel propagate downstream so the pipeline collapses
        // cleanly on the first failing stage.
        let upstream = match incoming_value_rx.map(|rx| rx.recv()) {
            None => None,
            Some(Ok(Ok(v))) => Some(v),
            Some(Ok(Err(e))) => {
            ...
            Some(Err(_)) => None, // sender dropped without sending; treat as None
        };

        let result = super::invoke::invoke(&stage_comp, upstream, child_env);
        if let Some(tx) = val_tx {
...
pub(super) fn launch_stage(
    ...
    if ctx.spec.dispatch.is_external() {
        ...
        let (handle, outgoing) =
            launch_internal_stage(stage, ctx.spec, is_last, shell, incoming, ctx.cancel)?;
