...
pub fn run_shell(
    ...
    let stdout_stream = Arc::new(StreamingPrinter::stdout(tx.clone()));
    let stderr_stream = Arc::new(StreamingPrinter::stderr(tx.clone()));
...
/// Line-buffering writer used as the streaming branch of the
/// stdout/stderr `Sink::Tee`.  Bytes accumulate until a newline; each
/// completed line is sent as a [`UiEvent::ToolStdoutLine`] /
/// [`UiEvent::ToolStderrLine`] to the main UI thread, which renders
/// it with the same lime-rail / tinted-body styling the post-run
/// `ToolResult` frame uses.
///
/// Owning the line discipline at the producer end keeps each line
/// atomic on the wire — two sinks (one per stream) can interleave at
/// line boundaries but never mid-byte.
...
impl StreamingPrinter {
    pub fn stdout(tx: Sender<UiEvent>) -> Self {
        Self { kind: Stream::Stdout, tx, pending: Mutex::new(Vec::new()) }
    }

    pub fn stderr(tx: Sender<UiEvent>) -> Self {
        Self { kind: Stream::Stderr, tx, pending: Mutex::new(Vec::new()) }
    }
    ...
    fn emit_line(&self, line: &[u8]) {
        ...
        let lines = match self.kind {
            Stream::Stdout => ui::tool_stdout_line(&text),
            Stream::Stderr => ui::tool_stderr_line(&text),
        };
        ui::emit(&self.tx, lines);
    }
}
...
impl ExternalWrite for StreamingPrinter {
    fn write(&self, bytes: &[u8]) -> io::Result<()> {
        ...
        while let Some(pos) = pending.iter().position(|&b| b == b'\n') {
            ...
            // `line` includes the trailing '\n'; strip it so the event
            // body carries just the text, no newline.
            let body = line[..line.len() - 1].to_vec();
            // Drop the lock around the send so a sibling stream can
            // buffer concurrently while we emit.
            drop(pending);
            self.emit_line(&body);
