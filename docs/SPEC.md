...
The following is a byte pipeline. Its stages may be builtins, external
commands, or ral code, but each interior handoff is bytes:

```ral
to-json $record | jq '.name' | from-line
```

An external command cannot receive a structured value by accident. The encoder
...
```ral
let items = range 0 5
let squares = map { |n| return $[$n * $n] } $items
```
...
An interior edge is dead once its reader stage has ended. A stage feels that death at exactly one place — its next write to that edge — and nowhere else. A ral-written stage's own write to a dead edge raises the break at that write; an external's write ral cannot see, so ral hears it instead: once the reader is gone, ral reads the edge itself, on the read-end duplicate it already holds, discards whatever bytes are pending at that moment (they were owed to a reader that left), and treats the first byte written after them as the kill's cue — SIGKILL on Unix, a distinctive-code `TerminateProcess` on Windows. A write already blocked on a full pipe when the edge dies is completed into that same read and then judged dead: a write not complete when the edge dies does not complete.

That break — and only that break — is the pipeline's one forgiven death: a non-final stage ral cuts this way keeps no failure, because the rest of its output was owed to nobody. Everything a stage does before its first dead write is its own account and runs to completion: its stderr, its files, its children that never write the edge, its own exit status if it never writes again. Nothing after that write is promised. The same condition in the final stage remains its own failure: the final stage's output crosses the pipeline's outer boundary, not an interior edge, so a caller that stops reading the pipeline's own output is a failure that stage reports for itself.

A stage whose own redirect statically sends every byte of its stdout to a file, never to the interior edge, never performs the write the cut watches for: `cmd > file | next` therefore runs `cmd`'s redirect to completion regardless of when `next` settles, not because it is exempted but because it never reaches the operation the rule cuts.

ral holds a duplicate of each interior edge's read end until that edge's writer stage is reaped or joined, so no interior edge ever delivers a broken-pipe signal or a write error to the stage that writes it. For a ral-written stage the cut is exact — its own write is the operation that raises the break. For a process it lands within the kill's few-microsecond latency after the first dead write. An exit status, once recorded, is never overwritten: the kill precedes the wait, and a kill landing on an already-exited stage changes nothing. This is one rule, stated once, true on both platforms.

A producer that exits on its own account keeps that status, whatever the cause: `sh -c 'exit 3' | true` still fails with status 3, since it never writes to the dead edge and so is never cut, while `yes | head -1` still succeeds, since its next write past the cut is the forgiven death — the same rule on both platforms rather than a broken-pipe signal on one and an exit-order reading on the other. A producer that never writes again is never cut and runs to completion: `sleep 1000 | true` waits for the sleep, as in every shell. One that must not be cut at all must not be a stage whose reader can leave — its own statement, or `spawn`.
...
One write no grant admits: onto a binary the sandbox pinned at startup — bubblewrap on Linux, ral’s own executable where it re-execs itself — judged by inode, so a hard link or a rename since startup names it too. The refusal names the binary and is not a grant denial. Replacing the file by rename is permitted; the session keeps running the copy it pinned.
...
it lives in an error record, a diagnostic, or an observation's separate `error`
field.

Byte pipelines preserve the same separation: their edges transport bytes
between stages, while diagnostics and audit data use their own channels. A
pipeline edge is never an implicit diagnostic or audit channel.
...
These are value operations: ordinary composition uses application and bind. Higher-order
...
a byte pipeline still transports bytes at every interior edge, while ordinary
application and bind compose values. The frontend does not reinterpret the
pipeline operator.
...
The ral language and the `ral` executable are separate contracts. The language defines ral source, values, byte pipelines, and effects. The executable chooses where source comes from, prepares a session, reports diagnostics, and maps the result to an operating-system exit status.
...
exchange bytes, while final values are reported through the ral result boundary.
Explicit adapters perform any conversion between values and bytes.
...
final ral result boundary or ordinary application/bind, and explicit adapters
perform conversions between values and bytes. Paths, executable lookup,
path-list separators, default system paths, and sandbox enforcement otherwise
follow the host rules described in their respective sections.
