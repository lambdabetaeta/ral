...
is judged on the `surface` form the author wrote.** Neither form is the "real"
one and the other a spelling of it.
...
- **fs — `path::covers` / `PrefixSet::covering`, on `resolved`.** An `fs` grant
  ...
  resolve").
- **exec — `NormalizedPrefix::covers_name`, on `surface`.** The question
  ...
  `allow_dirs` / `deny_dirs` prefix is therefore compared as written.

Exec does not thereby launder a symlink: the broadening happens on the
*candidate* side instead. An allow dir sees only the narrow spellings a head
earns, a deny dir additionally sees the canonical one, so a binary symlinked out
of a denied directory into an allowed one is still vetoed
([[decisions/260602_exec-authority-partitioned|exec-authority-partitioned]]).
Asking the *prefix* on its resolved form instead would silently re-aim a written
rule at whatever the disk happened to say at freeze — an exec rule names a name.
...
- `same_gate_dir` is mutual `covers_name`, on the surface — it exists to mirror
  the exec gate, so it judges the form that gate judges.
