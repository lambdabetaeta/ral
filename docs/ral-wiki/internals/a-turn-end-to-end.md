...
anchors: [run_source_turn, run_value_turn, TurnRequest, TurnReport, TurnIo, TurnState, TurnGuard, compile_turn, build_turn, run_framed, eval_top_level, Mobile, compile_and_typecheck]
---

# A turn, end to end

**A *turn* is one top-level evaluation against a persistent `Shell`, and every
host starts it through one of two synchronous, runtime-agnostic doors** —
`Shell::run_source_turn(src, TurnRequest) -> TurnReport` for a program in source
text and `Shell::run_value_turn(thunk, args, src, TurnRequest) -> TurnReport`
for one the host already holds as a `Value`, both in
[`core/src/driver.rs`](../../../core/src/driver.rs). No host reimplements
evaluation; each is a *request supplier* that hands a door a `TurnRequest` and
...
reduction primitive behind the doors is crate-private, so a host cannot start an
...
**The spine the two doors orchestrate is one straight line**, owning resources
(`Sink`, `Source`, `TurnState`, guards, buffers) while the request describes
policy. The doors differ only in *which program runs* — the source door
compiles first; both then converge on `run_built` and the turn module's framed
scaffold:
...
- The source door's `compile_turn` runs
  ...
  treats it as status 1. The value door skips this: its program is an
...
- `run_framed` installs that state through a `TurnGuard` and evaluates. The
  ...
  capability ceiling — `eval_top_level(&comp, s)` for the source door, the
  in-frame `builtins::apply(&thunk, &args, s)` for the value door
...
**The hosts differ only in the request they supply, and which door they call.**
...
- The REPL's `execute_input` (`ral/src/repl/exec.rs`) supplies `script_name:
  ...
  `pre-exec` / `chpwd` / `post-exec` plugin hooks; it calls `run_source_turn`
  synchronously on its prompt thread and renders with `print_result`. Its plugin
  hooks and prompt body (`ral/src/repl/plugin.rs`, `prompt.rs`) call
  `run_value_turn` instead — a thunk the REPL already holds, run through the same
  frame.
- exarch's `run_shell` (`exarch/src/shell_eval.rs`) supplies `script_name:
  ...
  its presentation bus; it calls `run_source_turn` and renders the capped
...
- ral's batch path (`ral/src/main.rs`) supplies `TurnIo::Inherit`, no surface,
  ...
  the terminal — the third `run_source_turn` client, closing the one entry
