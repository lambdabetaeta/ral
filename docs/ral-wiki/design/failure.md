...
*failure* falls through to the next arm. A non-failure escape — `exit` — propagates out immediately instead of being caught. If every arm fails the last error propagates, and all arms must
