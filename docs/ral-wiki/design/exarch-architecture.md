...
**The sandbox is not invented; it is ral's.** Each turn evaluates under a
...
host installs the frame around the turn, and external commands route through the
...
shell-outs.** The top-level contract installs each turn's post-run state onto the
...
- **auto-compaction.** Long autonomous turns stay bounded — the history is
...
exarch adds only LLM transport and turn orchestration:
