...
durable cancel root, the source registry, the host builtin table, and the
terminal lease — `SessionState` ([[decisions/260617_turn-local-state|turn-local-state]]
names the four lifetimes). There is exactly one such store per session, and a
same-thread body inherits it by *being* the same evaluation, not by receiving a
copy.
...
`root`, and now `terminal_lease`). It is a hand-maintained allow-list with no
totality check: any `SessionState` field added later, or any existing one a
refactor forgets, is silently dropped to its `Default`. The lease is the worst
case of that default, because `Shell::new` does not leave it obviously
uninitialised — it actively *re-derives* it from a blank predicate and produces a
confidently-wrong `None`. That is exactly the inference-not-possession failure
...
identity; there is nothing to re-attach. A small, explicit *fold-back set* —
`last_status` always, the logical `cwd` for a lambda, nothing else — replaces the
two asymmetric `inherit_from` / `return_to` manifests. Lambda-body elimination
thereby converges on the in-place mechanism the block path
(`with_block` / `run_with_mobile`) already uses; `eval_block` already folds
`last_status` back, so only the bound parameter and the `cwd` entry are new.

The owned-`Shell` construction (`from_captured` / `child_of`) is **retained** for
the cases that are genuinely a *different* runtime and so must copy the store:

- a spawned worker on another OS thread (`spawn_thread`: `spawn`, `par`, pipeline
  stages);
...
- **New:** one in-place thunk-body elimination on `Shell` (generalising
  `with_block`), parameterised by the bound pattern (lambda) or none (block) and
  by the fold-back set.
- **Changed:** `apply_lambda_frame` stops calling `with_child`; it swaps the
  mobile, binds the parameter in the pushed frame, and folds back
  `{last_status, cwd}`. `with_child` loses its only caller and is removed; the
  terminal-lease move added to `inherit_from` / `return_to` as the way-station is
  removed with it.
- **Retained:** `from_captured` and `child_of` for the owned-`Shell` modes —
  `spawn_thread`, `child_eval`, the REPL aside `child_from`, and the per-call
  overhead bench; the CBPV value-level `Block` / `Lambda` split and their distinct
  eliminations; `Shell::new` minting the lease at *real* session construction.
...
```
1  In-place lambda   add the generalised in-place body routine; route
                     apply_lambda_frame through it with the parameter bound and
                     {last_status, cwd} folded back. Behaviour-preserving.
2  Retire the copy   remove with_child and the terminal-lease move from
                     inherit_from / return_to; confirm child_of is reached only by
                     child_eval, child_from, spawn-adjacent code, and the bench.
3  Flow-matrix audit confirm read-once stdin, audit nesting, and the source cursor
                     behave identically under sharing; pin each with a test.
```
...
- **Block parity preserved.** `!{ … }` still discards `let` / `cd`; a lambda
  body's `cd` still persists; both share the audit trail and stdin.
