...
## [2026-06-23] query | pins filed as a design concept page

Answered "how do pins work in exarch?" and filed the answer back as [[design/pins|pins]]. Read the landed implementation: the `` `pin ``/`` `unpin `` disposition wrapper decoded by `value_to_pin` (`card.rs`), tried first on the surface channel in `AgentSink::emit` (`shell_eval.rs`) ahead of the `io`/`card` arms; `Kind::Pin`/`Kind::Unpin` (`bus.rs`) routed directly to the `Viewport`'s ordered `key → Card` register (`set_pin`/`drop_pin`, `viewport.rs`), drawn as the focused session's reserved right-hand column (`tui.rs`), neither logged nor landed in scrollback; headless drops it (`headless.rs`). Also traced the nudge tie-in: the session keeps a pin-blind `PinDigests` mirror (`shell_eval.rs`) so the [[map/exarch/session|nudge]] facility can fire a budget-free periodic reminder naming the model's pinned state. The page leads with the thesis (a pin is *state*, the model-authored dual of the agent matrix) and links down to the code maps, across to [[decisions/260622_surface-pins-state|surface-pins-state]] (the accepted ADR, which the index had not yet listed — now added), and to [[decisions/260618_tui-transcript-as-graphic|transcript-as-graphic]] for the encode-don't-stream doctrine pins finally let a kit obey. Updated [[index|index]].

## [2026-06-24] ingest | AGENTS.md injection + exarch config dir

Filed two `design/` concept pages from the prompt-assembly and config landing. [[design/exarch-config-dir|exarch-config-dir]] records exarch's XDG split — trusted config home (`config.ral`, global `AGENTS.md`) versus the per-project state home (model selection + run logs) and the cache home — and the structural trust model: the config home is trusted because the [[design/grant|grant]] cannot reach it, yet `config.ral` is still evaluated no-authority because a redirected endpoint is an exfiltration channel. [[design/agents-md-injection|agents-md-injection]] records the `Workspace` section: `AGENTS.md` discovered outermost-first (operator config dir, then git-root-down-to-cwd, stopping at the first `.git` entry), steering behaviour with prompt text alone — a cwd file is untrusted yet harmless because it cannot widen the grant, the mirror of trusted `config.ral`'s effect-bearing no-authority defence.

## [2026-06-24] ingest | mandatory permissions base narrows a spawned agent

Ingested `129914e`: the [[map/exarch/tools|`agent` tool]] gains a *mandatory* `permissions` parameter naming one of the five capability bake-ins, and a spawned child is born with `parent ⊓ resolve_base(permissions)` — a lattice meet that can narrow a child below the parent but never escalate it past the parent's authority (`dangerous` is the top, meaning inherit verbatim). The composition lives in `policy::narrow`, the meet-sibling of the root's `policy::for_invocation`; [[map/exarch/session|`Session::fork`]] now takes the child's `Capabilities` as an argument so the spawn site owns the decision. Filed a new concept page [[design/agents|agents]] consolidating the whole sub-agent model — the three roles (interactive root / headless returning root / peer) on the two orthogonal axes (`spawns`/`replies`), value-snapshot spawning on a detached thread bounded depth-1, the deliberate `reply` return, inherited self-scheduling authority, and the permissions narrowing — cross-linking the detail pages rather than duplicating them. Refreshed [[map/exarch/tools|tools]], [[map/exarch/policy|policy]], and [[map/exarch/session|session]] (re-stamped to `129914e`/2026-06-24) and [[map/exarch|exarch]]; updated [[index|index]].

## [2026-06-24] ingest | credential env scrub on the exarch front door

Recorded the startup env scrub on [[map/exarch|exarch]] (new "Credentials and the env scrub" section; added `exarch/src/credential.rs` to `covers_paths`, re-stamped to `f1c187e`/2026-06-24). `CredentialStore::resolve_and_scrub` reads each provider's key variable into the in-memory store, then removes from the process environment every key variable that was present — valid or malformed — so no child a tool call spawns inherits a live key; the removal also makes resolution eager. Framed as the subtractive half of exarch's env shaping against the additive `Scratch`/`NO_COLOR` seeding, with per-spawn loader-variable hardening noted as [[map/core|ral-core]]'s; the *why* stays in [[decisions/260613_provider-config-ral-script|provider-config-ral-script]]. Updated [[index|index]].

## [2026-06-24] ingest | uniform agent nodes (fleet / agent split)

Filed [[decisions/260624_uniform-agent-nodes|uniform-agent-nodes]] (*proposed*) from a design conversation: split the role-conflating `Session` into a thin **`Fleet`** (registry + event bus + `focus` + `interactive`) and a uniform **`Agent`** carrying `parent: Option<AgentId>` and an owned `ProviderHandle`. Every agent spawns (tree unbounded in depth — supersedes the depth-1 cap and `spawns()` axis of [[decisions/260617_async-agent-tool|async-agent-tool]]); the parent-less trunk is the sole distinguished node, with `replies()` and `park_idle()` derived from `parent = None ∧ interactive` (refining [[decisions/260623_reply-terminates-returning-agents|reply-terminates-returning-agents]], deleting `is_root`/`park_when_idle`). Dynamic attachment by `TAB`/`focus`, `Esc` + `agent_cancel` cascading to the subtree (generalising [[decisions/260612_per-root-turn-cancel|per-root-turn-cancel]]), liveness as `registry non-empty` with no human-less daemon. Carries a six-phase realisation plan; `expect_action` dropped. Updated [[index|index]]. Map/design re-stamps ([[design/agents|agents]], [[map/exarch/session|session]], [[map/exarch/tools|tools]]) land with implementation, not this proposal.

## [2026-06-24] ingest | uniform agent nodes landed (fleet / agent split)

Implemented [[decisions/260624_uniform-agent-nodes|uniform-agent-nodes]] (now *active*); ingested the landed code at `3e5ce15`. `git mv session.rs → agent.rs`; `Session → Agent`, `SessionLog → AgentLog`, `SessionBus → FleetBus`, `SessionId` canonicalised to `AgentId` (`pub type AgentId = u64`, `bus.rs`). New thin `fleet.rs` — `{ agents, bus, focus: Arc<AtomicU64>, interactive }`, `alive() ⟺ registry non-empty`, `NO_FOCUS` sentinel. The trunk is the parent-less node: `is_root`/`park_when_idle`/`expect_action`/the per-turn `acted` flag are deleted, `returns()` reads as `¬(parent = None ∧ interactive)`, and parking is a `ParkMode` (`Held`/`UntilCancelled`/`Quiesce`) computed in `Agent::park_mode`, re-checked on every inbox wake so a `TAB`-away flips a de-focused agent to `Quiesce`. `ToolSet` lost its `spawns` axis — spawning is universal (tree unbounded in depth), `Tool::spawns()` gone, the two constructors are `ToolSet::conversing()` (no `reply`) and `ToolSet::returning()` (with `reply`). Provider is per-agent and hot-swappable: `/model` swaps the focused agent's handle, `fork` seeds the child's from the parent's current provider. `AgentRegistry::Entry` gained a `parent` link and a per-agent provider; `cancel(id)` cascades the subtree and `clear_subtree(root)` reaps + bumps generation — one cascade serving `agent_cancel`, the per-agent ceiling, and `Esc`; the trunk publishes its sticky token via `cancel::publish`, cleared each turn by `Token::reset`. Re-stamped [[design/agents|agents]] (two axes → the one `parent` predicate), `map/exarch/session` → [[map/exarch/agent|agent]] (`git mv`, rewritten for the uniform node + Fleet + drive loop + subtree cascade, `@3e5ce15`), [[map/exarch/tools|tools]] (one `replies` axis, universal `agent` family, `conversing()`/`returning()`, `@3e5ce15`), and [[map/exarch|exarch]] (`@3e5ce15`); retargeted the wiki-links from the renamed page across the live map/design/invariant/decision pages. `SessionInfo`, `SessionToolResult`, and the `event.rs` log-schema types (`SessionEvent`/`SessionStarted`/`SessionEnded`) keep their names. Updated [[index|index]].

## [2026-06-25] query | edit-harness comparison folded into hash-addressed-editing

Read exarch's `edit` against two anchor-based coding-agent harnesses and rewrote the Reference section of [[design/hash-addressed-editing|hash-addressed-editing]] as a comparison: Bölük's [“The Harness Problem”](https://blog.can.ac/2026/02/12/the-harness-problem/) (*hashline* — `line#:hash` tags, line number for uniqueness) and Dirac's [hash-anchors + Myers diff](https://dirac.run/posts/hash-anchors-myers-diff-single-token) (pre-generated single-token anchors over a five-component state machine reconciled by Myers diff). All three cut edit output to `O(replacement)` and reject on drift; exarch's ±3-context window-hash beats hashline's brittle line number on identity and reaches Dirac's efficiency without its stateful machine, paying a wider anchor and no cross-edit stability (blunted by the atomic batch). Updated [[index|index]].

## [2026-06-27] ingest | macOS sandbox profile records tty-ioctl hole

Refined the macOS Seatbelt base profile in [[map/core/capabilities|capabilities]]: `apple.shm.notification_center` is now rendered in the POSIX shared-memory namespace, matching Apple's profiles. Left the broad `file-ioctl` allowance in place for sandboxed full-screen TUI children and recorded the known hole: the current `SandboxProjection` has no terminal-loan bit, so the profile cannot yet deny `/dev/tty` ioctls for ordinary Denied-terminal turns while admitting them for explicit terminal loans.

## [2026-06-28] ingest | host seam made transport-parametric (front-end owns terminal, engine leases it)

Filed [[decisions/260628_host-seam-transport-parametric|host-seam-transport-parametric]] (*proposed*) from a design conversation about a future where a front-end (an interactive shell, or `exarch`) runs on a host while the `ral` engine runs inside a VM. The decision makes the `Shell::run_turn` seam ([[decisions/260618_run-turn-is-host-api|run-turn-is-host-api]]) *parametric in its transport*: the same request/result/cancel/lease vocabulary runs as an in-process call sharing a kernel or across a process/VM boundary, the local case being the *identity* transport rather than a privileged path. The pivot is **ownership ≠ operation**: a front-end owns a controlling terminal; the engine never owns one, it operates a terminal it is *granted* — a real kernel tty that is the login tty (identity transport) or a PTY (remote), and because a PTY is a tty the engine's `tcsetpgrp`/termios/pgid code does not branch. Three corollaries carry the weight: (1) the [[decisions/260619_terminal-lease|terminal lease]]'s authority is conveyed across the seam by the terminal's owner, replacing `mint_at_startup`'s local `tcgetpgrp` self-mint and so completing the [[decisions/260601_reduced-authority-witness|reduced-authority-witness]] discipline the lease left half-done (the carrier — session `Option<TerminalLease>` + per-turn `TerminalAccess` — is unchanged; only provenance and referent move); (2) POSIX signals live only within a process and cross the seam as typed `cancel`/`suspend`/`resize` messages, deleting commit `6abb905`'s `RAL_SIGINT_HANDLER`/`RAL_TERM_HANDLER` chaining *at the seam*, over a full-duplex transport so the since-removed `sandbox-ipc-cancel` out-of-band-cancel hard rule (a wedged synchronous round trip is uncancellable) is reinstated as a law; (3) the cut falls only around a whole turn, never inside an effect scope — the legitimate whole-interpreter boundary [[decisions/260617_sandbox-external-children|sandbox-external-children]] preserved, explicitly *not* the grant-body IPC it removed (the `WireMobile`/`run_confined` mistake of bisecting a grant meet). The engine is a *persistent* process, so a transport disconnect mid-lease cancels the in-flight turn and reaps its foreground child but loses no session state, and a front-end may re-attach to the live engine — session continuity is not a per-turn serialisation problem because turns and leases cross the wire while the dynamic context never does (this is also why process-per-turn is rejected). `exarch` stays only an agent; the `ral` shell host is its peer; both drive one engine over one seam. The ADR records the lease state machine (Denied/Leased/ExplicitLoan over a turn, with foreground acquire/release and suspend/resume) and pins the disconnect-mid-lease corner; open questions are surface/notify wire framing ([[decisions/260622_sync-surface-async-notify|sync-surface-async-notify]]), a terminal-loan bit in `SandboxProjection` (the tty-ioctl hole noted 2026-06-27), and reconnect identity. Nothing built; rests on landed pieces. Updated [[index|index]].

## [2026-06-29] ingest | dedent preserves content-line whitespace

Fixed `dedent` in `core/src/builtins/strings.rs` so multiline raw-block framing is removed line-wise instead of by a whole-block trim: blank opener/closer lines fall away, the common margin is stripped, and relative indentation on the first content line plus trailing spaces on the last content line survive. Re-stamped [[map/core/builtins|builtins]] to `1c2a010` and added regression coverage for the first-line raw-string case.
