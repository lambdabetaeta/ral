...
//
// PATCH (vendored fork): upstream builds this client against
// `rustls-platform-verifier`, which loads zero roots and aborts the build on
// hosts with no system trust store (e.g. a container image shipping no
// `ca-certificates` bundle). This is the only transport behind genai's
// model-listing adapter paths, which take no injectable client. Bind it to the
// bundled Mozilla webpki roots, matching exarch/src/tls.rs.
...
/// rustls config validating against the bundled Mozilla webpki roots, with the
/// aws-lc-rs provider — the same rustls version and provider reqwest is built
/// with, so reqwest's `use_preconfigured_tls` downcast succeeds at build time.
