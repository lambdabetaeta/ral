...
/// Parse explicit CLI arguments and convert them to their high level
/// representation. `rawargs` must not include argv[0].
pub(crate) fn parse_from<I, O>(rawargs: I) -> ParseResult<HiArgs>
where
    I: IntoIterator<Item = O>,
    O: Into<OsString>,
{
    let rawargs: Vec<OsString> = rawargs.into_iter().map(Into::into).collect();
    parse_low_from(rawargs).and_then(|low| match HiArgs::from_low_args(low) {
...
fn parse_low_from(rawargs: Vec<OsString>) -> ParseResult<LowArgs> {
    ...
    if let Err(err) = parser.parse(rawargs.clone(), &mut low) {
        return ParseResult::Err(err);
    }
