...
where
    ...
    O: Into<OsString>,
{
    finish(run(flags::parse_from(rawargs)))
}

/// Run the vendored ripgrep core with process argv (`std::env::args_os`).
pub fn run_env() -> u8 {
    finish(run(flags::parse()))
}

fn finish(result: anyhow::Result<u8>) -> u8 {
    match result {
        Ok(code) => code,
        Err(err) => {
