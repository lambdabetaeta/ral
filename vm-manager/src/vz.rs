...
fn boot_machine(
    ...
    on_queue(queue, move || {
        ...
        // One `VZVirtioSocketDeviceConfiguration` already multiplexes every
        // port, so this is two listener registrations on the one device, not
        // a second device.
        unsafe {
            ...
            let completion = RcBlock::new(
                move |connection: *mut VZVirtioSocketConnection,
                    ...
                    let outcome = if let Some(connection) = connection.as_ref() {
                        ...
                        Err(format!(
                            "the guest refused the dial: {}",
                            error.localizedDescription()
                        ))
                    } else {
                        Err(
                            "the connect completed with neither a connection nor an error"
                                .to_string(),
                        )
                    };
